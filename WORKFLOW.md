...
## Developer Notes (build + structure)
- Author pages in `src/pages/<day>.njk` using the shared layout `templates/layouts/day.njk` (set front matter, then fill `{% block styles %}` and `{% block body %}`; layout injects head/nav/footer scripts).
- Shared assets live under `site-assets/`; navigation uses `days.json` generated by the build.
- Build commands:
  - Preview (safe): `npm run dev` (`build:pages:check` → `maps-build/`)
  - Publish build: `npm run publish` (`build:pages` + `node scripts/build-site.mjs 26` to set Day 26 as homepage redirect)
  - Serve locally: `npm run dev:server` (serves `docs/` by default; add `--dir .` and/or `--port 3000` if needed)
- Builders:
  - `scripts/build-days.mjs` renders NJK pages to HTML.
  - `scripts/build-site.mjs` copies `maps/` to `docs/` and writes `days.json` + `docs/index.html`.
