...
| --- | --- | --- | --- |
| Google AI Studio Apps | A- | Excellent! Built sophisticated map with dual-source data, generative AI calligraphy art, country masking, caching. Required minimal fixes, mostly feature additions. | Added more cities/languages, refined prompts for image generation, implemented country masking feature |
...
| Claude Artifacts | C- | Was working but got stuck at loading when trying to fetch data from API. | Attempted to get it to fetch data from API |
...
Notes:
- Bolt.new: not run (free tokens exhausted for this day).
- All platforms attempted to use Overpass API but none succeeded - all fell back to hardcoded data. Curious why even Gemini 3 didn't get Overpass working.
